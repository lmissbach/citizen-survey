...
conjacp.estimation <- function(conjacp.prepdata.object,
  ...
  for (attr_temp in attr_x[attr_x != attr_cond]) {
    ...
    for (sub in unique(data_prep$subgroup)) {
      ...
      # Adjustement
      if (adjust == FALSE) {
        c_idx <- which(substr(colnames(data_model), 1, 2) == "c_")
        if (length(c_idx) > 0) {
          data_model <- data_model[, -c_idx, drop = FALSE]
        }
      }
      ...
      if (attr_temp %in% attr_unrestricted) {
        ...
        meta_drop    <- match(c("clust", "id", "subgroup"), names(data_model))
        meta_drop    <- meta_drop[!is.na(meta_drop)]
        if (length(meta_drop) > 0) {
          data_model <- data_model[, -meta_drop, drop = FALSE]
        }
      ...
      # For constrainted attributes, run two regressions:
        ...
        for (r in 1:2) {
          ...
          meta_drop       <- match(c("clust", "id", "subgroup"), names(data_model_cacp))
          meta_drop       <- meta_drop[!is.na(meta_drop)]
          if (length(meta_drop) > 0) {
            data_model_cacp <- data_model_cacp[, -meta_drop, drop = FALSE]
          }
  ...
  get.full.vcov <- function(models) {
    ...
    # Calculate covariance matrices (skip when only one attribute model)
    if (length(models) < 2) {
      return(full_vcov)
    }
...
# Patched for single-attribute V-tables (see R/conjoint_acp_helpers.R).
